...
@section('content')
    <div class="container">
        ...
        <div class="card card-custom gutter-b example example-compact">
            ...
            <!--begin::Form-->
            <form class="form" method="post" action="dash/roles/{{$role->id}}">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="form-group">
                        <label>Name:</label>
                        <input type="text" name="name" class="form-control form-control-solid" value="{{$role->name}}" placeholder="Enter role name" required/>
                    ...
                    <div class="form-group">
                        <select id="rolePermissions" multiple="multiple" name="rolePermissions[]">
                            @foreach($permissions as $permission)
                                <option value="{{$permission->id}}" {{$rolePermissions->contains($permission->id) ? "selected" : ""}}>{{$permission->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
