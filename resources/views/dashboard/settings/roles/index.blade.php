...
@section('scripts')
    ...
    <script>
              ...
              columns: [
                  ...
                  },
                  {
                      ...
                      @if(Auth::user()->can('edit-role') || Auth::user()->can('delete-role'))
                          @can('delete-role')
                              <a href="dash/roles/' + row.id + '/edit" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details">\
	                      ...
	                      @endcan
                          @can('edit-role')
                                <a href="#" data-toggle="modal"  data-target="#deleteModal" data-role_id="' + row.id + '" data-role_name="' + row.name + '" class="btn btn-sm btn-clean btn-icon" title="Delete">\
                                    <span class="svg-icon svg-icon-md">\
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
                                                <rect x="0" y="0" width="24" height="24"/>\
        ...
        $('#deleteModal').on('show.bs.modal', function(e) {
            ...
            $(e.currentTarget).find('#exampleModalFormTitle').text('Do you really want to delete the permission '+role_name+' ?');
            $(e.currentTarget).find('#deleteForm').attr('action', 'dash/roles/'+role_id);
        });
        //---------------------------------------------------------------------------------------
    ...
    <div class="container">
        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-3">
                <div class="card-title">
                    <h3 class="card-label">Roles <span class="d-block text-muted pt-2 font-size-sm">Be careful</span></h3>
                </div>
                ...
                @endcan
                    <!--begin::Button-->
                @can('create-role')
                    <a href="dash/roles/create" class="btn btn-primary font-weight-bolder">
                        <span class="svg-icon svg-icon-md">
            ...
            <div class="card-body">
                <form id="deleteMultiForm" action="dash/roles/delete-multi" method="post">
                @csrf
                <!--begin: Datatable-->
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="exampleModalFormTitle" aria-hidden="true" style="display: none;">
            <div class="modal-dialog" role="document">
                <form id="deleteForm" action="dash/roles/{id}" method="post">
                    @csrf
                    @method('delete')
        ...
        <div class="modal fade" id="deleteMultiModal" tabindex="-1" aria-labelledby="exampleModalFormTitle" aria-hidden="true" style="display: none;">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalFormTitle">Do you really want to delete these roles ?</h5>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light-primary font-weight-bold" data-dismiss="modal">Close</button>
